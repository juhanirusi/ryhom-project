...
class Article(BaseAbstractModel):
    ...
    status = models.CharField(
        max_length=16, choices=Status.choices, default=Status.WRITER_SAVED_FOR_LATER
    )
    ...
    class Meta:
        db_table = "article"  # Good to define table name!
...
class ArticleComment(BaseCommentModel):
    ...
    likers = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name="article_comments_likes"
    )
    ...
    class Meta:
        db_table = "article_comment"  # Good to define table name!
